...
module.exports = async (fastify, opts) => {
	fastify.get('/', async function (req, res) {
		...
		const result = await cursor.find({}).sort({timestamp: -1}).skip((limit * (page - 1)) - (page-1)).limit(limit).toArray();// we take all items, sort them, limit them, skip a limited and return array
